...
class QuranProvider extends ChangeNotifier {
  ...
  final Map<String, List<dynamic>> _allTranslations = {}; // সব ভাষার অনুবাদ
  ...
  Future<void> initQuran() async {
    ...
    try {
      // ১. আরবী ফাইল লোড
      // debugdebugPrint("📖 Loading Arabic Quran...");
      ...
      // debugdebugPrint("✅ Arabic loaded: ${_surahs.length} surahs");

      // ২. সব ভাষার অনুবাদ লোড
      await _loadAllTranslations();

      _isLoading = false;
      // debugdebugPrint("🎉 All data loaded successfully!");
    } catch (e) {
      _isLoading = false;
      // debugdebugPrint("❌ Error loading data: $e");
    }
  ...
  Future<void> _loadAllTranslations() async {
    ...
    for (String code in languageCodes) {
      try {
        // debugdebugPrint("🔄 Loading $code translation...");
        ...
        for (int i = 1; i <= 114; i++) {
          ...
          if (jsonMap.containsKey(surahKey)) {
            ...
            List<Map<String, dynamic>> ayahList = [];
            for (int j = 0; j < ayahs.length; j++) {
              ayahList.add({'number': j + 1, 'text': ayahs[j]['text'] ?? ''});
            }

            translationList.add({'number': i, 'ayahs': ayahList});
          }
        }

        _allTranslations[code] = translationList;
        // debugdebugPrint("✅ $code loaded: ${translationList.length} surahs");
      } catch (e) {
        // debugdebugPrint("❌ Error loading $code: $e");
  ...
  Future<void> changeLanguage(String langCode) async {
    ...
    if (_allTranslations[langCode] == null ||
        _allTranslations[langCode]!.isEmpty) {
      // debugdebugPrint("⚠️ $langCode translation not available");
      return;
    }

    _currentLang = langCode;
    notifyListeners();
    // debugdebugPrint("🔄 Language changed to: $langCode");
  }
