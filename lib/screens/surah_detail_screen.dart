...
class _SurahDetailScreenState extends State<SurahDetailScreen> {
  ...
  // ✅ প্রতিটি আয়াতের জন্য আলাদা Key লিস্ট
  ...
  void initState() {
    ...
    // আয়াত বাজার সাথে সাথে হাইলাইট এবং অটো-স্ক্রল করার লিসেনার
    _audioPlayer.currentIndexStream.listen((index) {
      if (index != null && _cachedAyahs.isNotEmpty && mounted) {
        ...
        // হাইলাইট লজিক (আপনার আগের কোড)
        if (surahNumber == 1) {
          if (index >= 0 && index < _cachedAyahs.length) {
            setState(() {
              _currentlyPlayingAyah = _cachedAyahs[index]['globalAyahId'];
            });
          } else {
        ...
        if (index == 0) {
          ...
          if (ayahIndex >= 0 && ayahIndex < _cachedAyahs.length) {
            setState(() {
              _currentlyPlayingAyah = _cachedAyahs[ayahIndex]['globalAyahId'];
            });
          } else {
            setState(() => _currentlyPlayingAyah = -1);
          }
  ...
  void _autoScrollToIndex(int index) {
    ...
    // বিসমিল্লাহ থাকলে ইনডেক্স অ্যাডজাস্টমেন্ট
    if (surahNumber != 1 && surahNumber != 9) {
      if (index == 0) {
        // বিসমিল্লাহ বাজলে একবারে উপরে চলে যাবে
    ...
    // নির্দিষ্ট আয়াতে নিখুঁতভাবে স্ক্রল করা
    if (targetKeyIndex >= 0 && targetKeyIndex < _ayahKeys.length) {
      ...
      if (keyContext != null) {
        Scrollable.ensureVisible(
          ...
          alignment: 0.1, // আয়াতটি স্ক্রিনের একটু নিচে/মাঝামাঝি রাখবে
        );
      }
    }
  }
  ...
  void _loadAyahs() {
    ...
    // ✅ আয়াতের সংখ্যা অনুযায়ী Key লিস্ট তৈরি
              ...
              headerSliverBuilder: (context, innerBoxIsScrolled) {
                return [
                  SliverAppBar(
                    ...
                    leading: IconButton(
                      icon: Icon(Icons.arrow_back_ios, color: innerBoxIsScrolled ? Colors.white : Colors.grey),
  ...
  Widget _buildAyahItem(Map<String, dynamic> ayah, int index) {
    ...
    return Column(
      // ✅ Key যুক্ত করা হয়েছে নিখুঁত স্ক্রলের জন্য
      key: _ayahKeys[index],
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              CircleAvatar(radius: 14, backgroundColor: const Color(0xFF1B5E20), child: Text("$ayahNumber", style: const TextStyle(color: Colors.white, fontSize: 10))),
              const Spacer(),
              IconButton(
                icon: const Icon(Icons.share_outlined, size: 20, color: Color(0xFF1B5E20)),
                onPressed: () => ShareLogic.shareAyah(arabicText: ayah['arabic'], englishTranslation: ayah['trans'], surahName: widget.surahName, ayahNumber: ayahNumber, context: context),
              ),
              ...
              Stack(
                ...
                children: [
                  if (_isAudioLoading && isPlaying)
                    ...
                    IconButton(
                      icon: Icon(isPlaying ? Icons.pause : Icons.play_arrow_outlined, color: const Color(0xFF1B5E20), size: 24),
                      onPressed: isPlaying ? _togglePlayPause : () => _playFullSurahAudio(index),
                    ),
                ],
              ),
              const SizedBox(width: 15),
              Consumer<BookmarkProvider>(
                builder: (context, bookmarkProvider, child) {
                  final Map<String, dynamic> bookmarkData = {
                    'id': globalAyahId, 'number': globalAyahId, 'surahName': widget.surahName, 'numberInSurah': ayahNumber,
                    'ayahNumber': ayahNumber, 'text': ayah['arabic'] ?? '', 'arabic': ayah['arabic'] ?? '',
                    'translation': ayah['trans'] ?? '', 'trans': ayah['trans'] ?? '',
                  };
                  bool bookmarked = bookmarkProvider.isBookmarked(bookmarkData);
                  return IconButton(icon: Icon(bookmarked ? Icons.bookmark : Icons.bookmark_border_rounded, color: const Color(0xFF1B5E20), size: 24), onPressed: () => bookmarkProvider.toggleBookmark(bookmarkData));
                },
              ),
            ],
          ),
